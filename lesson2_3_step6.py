...
try:
    ...
    # переключаемся на работу со второй вкладку
    second_window = browser.window_handles[1] # нашли вторую вкладку
    browser.switch_to.window(second_window)  # перешли на вторую вкладку

    # подтверждение confirm-а
    #confirm = browser.switch_to.alert
    #confirm.accept()

    x_element = browser.find_element(By.XPATH, "//span[@id='input_value']") # поиск значения Х и подсчет функции из предусловия
    x = x_element.text
    print (x)
    y = calc(x)
